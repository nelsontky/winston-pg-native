...
class Postgres extends Transport {
  ...
  log(args, callback) {
    ...
    if (sqlStatement) {
      sqlStatement = {
        text: sqlStatement,
        values: ['now()', level, message, JSON.stringify(meta)]
      };
    }
    ...
    return pool.connect().then(client => client
      .query(sqlStatement || table.insert(table.timestamp.value('NOW()'), table.level.value(level), table.message.value(message), table.meta.value(JSON.stringify(meta))).toQuery())
      .then(() => {
        client.release();
